...
from Scorelens_Engine_V2.symbol_extraction import Barline, Clef, Sfn, Rest, SfnType, ClefType, RestType
from Scorelens_Engine_V2.note_group_extraction import NoteGroup
from Scorelens_Engine_V2.notehead_extraction import NoteHead, NoteType
from Scorelens_Engine_V2.utils import get_global_unit_size, get_logger, get_total_track_nums
...
class Measure:
    ...
    def get_key(self):
        ...
        # ── [V2] Clamp to valid Key enum range (-6 to 6) ─────────────────────
        count = max(-6, min(6, count))
        ...
        try:
            ...
            logger.warning("[Key] Invalid key count=%d — defaulting to C Major", count)
            return Key(0)
    ...
    def align_symbols(self):
        ...
        # The auto-rhythm balancing algorithm is currently hardcoded for 2 tracks.
        if track_nums != 2:
...
class AddInit(Action):
    ...
    def perform(self, parent_elem=None):
        ...
        # ── Time Signature (inferred from rhythm analysis) ──
        beats, beat_type = self.time_sig
        time_elem = SubElement(attr, 'time')
        beats_elem = SubElement(time_elem, 'beats')
        beats_elem.text = str(beats)
        beat_type_elem = SubElement(time_elem, 'beat-type')
        beat_type_elem.text = str(beat_type)
        logger.info("[MusicXML] Writing time signature: %d/%d", beats, beat_type)
...
# Standard time signature candidates (beats, beat_type)
TIME_SIG_CANDIDATES = [
    (4, 4),   # 4/4 — most common
    (3, 4),   # 3/4 — waltz
    (2, 4),   # 2/4 — march
...
def infer_time_signature(measures_dict: dict) -> tuple[int, int]:
    """Infer time signature by measuring actual total duration per measure.

    Strategy:
      1. For every measure (skipping the first to avoid anacrusis), compute
         the total note duration on track 0 (treble staff) by summing slot_duras[:,0].
      2. Fallback: sum Voice durations directly from measure.symbols (track 0 only).
      3. For each candidate time sig, count how many measures fit within +/-12% tolerance.
      4. Return the candidate with the highest score. Fallback to 4/4.

    Returns:
        (beats, beat_type) tuple -- e.g. (4, 4) or (3, 4).
    """
    measure_durations = []
    for grp_measures in measures_dict.values():
        for i, measure in enumerate(grp_measures):
            if i == 0:
                continue  # skip potential anacrusis / pickup bar

            total = 0

            # Primary: use pre-computed slot_duras (shape: n_slots x n_tracks)
            # slot_duras[:, 0] = treble track durations per time slot
            if measure.slot_duras is not None and measure.slot_duras.shape[0] > 0:
                total = int(np.sum(measure.slot_duras[:, 0]))

            # Fallback: sum Voice durations from symbols on track 0
            if total == 0:
                total = sum(
                    sym.duration
                    for sym in measure.symbols
                    if isinstance(sym, Voice) and sym.track == 0
                )

            if total > 0:
                measure_durations.append(total)

    if not measure_durations:
        logger.warning("[TimeSig] No measure durations found -- defaulting to 4/4")
        return (4, 4)

    logger.debug("[TimeSig] Measure durations sample: %s", measure_durations[:10])

    # Score each candidate
    best = (4, 4)
    best_score = -1.0
    for beats, beat_type in TIME_SIG_CANDIDATES:
        expected = (beats * DIVISION_PER_QUATER * 4) // beat_type
        tolerance = max(1, round(expected * 0.12))  # 12% tolerance
        matching = sum(1 for d in measure_durations if abs(d - expected) <= tolerance)
        score = matching / len(measure_durations)
        logger.debug("[TimeSig] %d/%d expected=%d score=%.2f", beats, beat_type, expected, score)
        if score > best_score:
            best_score = score
            best = (beats, beat_type)

    logger.info(
        "[TimeSig] Inferred %d/%d (confidence %.0f%%, n=%d measures)",
        best[0], best[1], best_score * 100, len(measure_durations)
    )
    return best
...
class MusicXMLBuilder:
    ...
    def build(self):
        ...
        # ── Infer time signature if not explicitly provided ──
        if self.time_sig is None:
            self.time_sig = infer_time_signature(self.measures)
        elif isinstance(self.time_sig, str):
            # Parse string like "4/4" into (4, 4)
            match = re.compile(r'(\d+)/(\d+)').match(self.time_sig)
            if match:
                self.time_sig = (int(match.group(1)), int(match.group(2)))
            else:
                self.time_sig = (4, 4) # Fallback

        # ── [V2] NEW: Verify and Fix Measure Durations using Time Signature ──
        from Scorelens_Engine_V2.visual_verifier import verify_measure_duration
        verify_measure_duration(self.measures, self.time_sig)

        Action.clear()
        if not self.measures or 0 not in self.measures or not self.measures[0]:
            logger.warning("[MusicXML] No measures found to build.")
            return
