...
from scorelens_engine import exceptions as E
from scorelens_engine.utils import get_logger
from scorelens_engine.bbox import find_lines, get_bbox, get_center
...
class Line:
    ...
    def slope(self) -> float:
        if not hasattr(self, "_slope"):
            setattr(self, "_slope", None)
        if self._slope is not None:
            return self._slope
        if len(self.points) == 0: return 0.0
        x_c = self.x_center
        y_c = self.y_center
        diffs = [(p[1]-x_c, p[0]-y_c) for p in self.points]
        self._slope = sum(d[0]*d[1] for d in diffs) / (sum(d[0]**2 for d in diffs) + 1e-9)
        return self._slope
...
class Staff:
    ...
    def unit_size(self) -> int:
        ...
        if len(self.lines) < 2: return 10.0
...
def init_zones(staff_pred, splits):
    ...
    mean_accum_x = np.mean(accum_x)
    if mean_accum_x > 0:
        accum_x = accum_x / mean_accum_x
...
def extract_line(pred, x_offset, line_threshold=0.8):
    ...
    if len(cc) > 1:
        ...
        max_gap = 10  # Default fallback if only 0 or 1 line found
...
def filter_line_peaks(peaks, norm, max_gap_ratio=1.5):
    ...
    # Filter by x-axis
    if len(peaks) < 2:
        return valid_peaks, [0] * len(peaks)
...
def further_infer_track_nums(staffs, min_degree=75):
    # --- NEW: Gap-based inference ---
    # Try to infer track numbers by looking at the vertical gaps between staves.
    # Staves within the same system are typically closer together than staves across systems.
    if staffs.shape[1] > 1:
        first_col = staffs[0]
        gaps = []
        for i in range(1, len(first_col)):
            gap = first_col[i].y_center - first_col[i-1].y_center
            gaps.append(gap)
            
        gaps_arr = np.array(gaps)
        # If the max gap is significantly larger than the min gap (e.g., > 1.5x)
        # it indicates there are different types of gaps (intra-system vs inter-system)
        if np.max(gaps_arr) > np.min(gaps_arr) * 1.5:
            # Simple clustering by thresholding at the mean
            threshold = np.mean(gaps_arr)
            track_count = 1
            for gap in gaps_arr:
                if gap < threshold:
                    track_count += 1
            ...
            # Verify if this track_count is consistent
            if len(first_col) % track_count == 0:
                logger.debug(f"Gap-based inference found {track_count} tracks per system.")
                return track_count

    # --- FALLBACK: Original Barline-based inference ---
    # Fetch parameters
    ...
    for i in range(1, 10):
        ...
        # Relaxed the heuristic: we don't need the long barline to appear in every single zone (staffs.shape[0])
        # Just appearing a few times (e.g., at the start of systems) is enough.
        # But we don't know how many systems there are yet.
        if valid_h > 0:
            num_track = max(num_track, i + 1)
        else:
            break
            
    # Cap num_track to the maximum possible staves
    num_track = min(num_track, staffs.shape[1])
    logger.debug(f"Barline-based fallback found {num_track} tracks.")
    return num_track
