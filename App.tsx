...
const App: React.FC = () => {
  ...
  // ═══ BGM: Disabled temporarily to fix audio system ═══
  // The new Audio() approach was breaking Tone.js AudioContext on iOS/Safari.
  // TODO: Re-implement using <audio> element rendered only on home screen.
  ...
  return (
    <div className={`flex flex-col h-[100dvh] w-full bg-transparent font-sans selection:bg-cyan-500/30 ${performanceMode ? 'perf-mode' : ''}`}>
      ...
      <div className={`flex-1 flex min-h-0 overflow-hidden relative ${nimoPosition === 'right' ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
        ...
        {/* BGM is now handled via new Audio() in useEffect — no JSX element needed */}
      </div>
    </div>
  );
};
