...
import YouTubeStudioPage from './YouTubeStudioPage';
...
const StudioPage: React.FC<StudioPageProps> = ({
  ...
  const [studioMode, setStudioMode] = useState<'composer' | 'arranger' | 'editor' | 'pianoroll' | 'youtube'>(initialStudioMode);
      ...
      <header className="h-14 bg-[#0c0c0e] border-b border-white/5 flex items-center justify-between px-4 z-[3000] shrink-0 gap-4 overflow-x-auto">
        ...
        <div className="flex items-center gap-4 shrink-0 mx-auto">
          ...
          <div className="flex bg-[#111] p-1 rounded-2xl border border-white/10 shrink-0">
            ...
            <button 
              onClick={() => setStudioMode('youtube')} 
              className={`flex items-center gap-1.5 ml-2 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${
                studioMode === 'youtube'
                  ? 'bg-gradient-to-r from-red-600 to-red-500 text-white shadow-[0_0_15px_rgba(239,68,68,0.4)] scale-105 z-10' 
                  : 'bg-red-500/15 text-red-400 border border-red-500/30 hover:bg-red-500/30 hover:text-white'
              }`}
            >
