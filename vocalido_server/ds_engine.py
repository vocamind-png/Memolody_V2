...
class DiffSingerEngine:
    ...
    def synthesize_phrase(self, notes, params=None):
        ...
        mixed_audio = None
        stems_audio = [None] * len(tracks)

        import concurrent.futures

        def render_track(i_track):
            i, track = i_track
            print(f"[DiffSingerEngine] 🎙️ Rendering Track {i+1}/{len(tracks)} ({len(track)} notes)...")
            return i, self._synthesize_single_track(track, bpm, params)

        with concurrent.futures.ThreadPoolExecutor(max_workers=min(4, len(tracks))) as executor:
            futures = [executor.submit(render_track, (i, track)) for i, track in enumerate(tracks)]
            for future in concurrent.futures.as_completed(futures):
                i, track_audio = future.result()
                if track_audio is not None:
                    stems_audio[i] = track_audio.copy()
                    ...
                    stems_audio[i] = np.zeros(1024, dtype=np.float32)
