...
class MeasurePattern(BaseModel):
    ...
    piano_rhythm: str = Field(description="Must be one of: 'block_whole', 'block_syncopated', 'arpeggio_up', 'arpeggio_down', 'offbeats', 'driving_8ths', 'bossa_nova_comping', 'edm_synth_chords', 'lofi_chords', 'acoustic_strumming', 'classical_arpeggio', 'rnb_neo_soul_chords'")
    bass_rhythm: str = Field(description="Must be one of: 'whole_note', 'half_notes', 'driving_8ths', 'syncopated_pop', 'walking', 'bossa_nova_bass', 'edm_offbeat_bass', 'lofi_sub_bass', 'rnb_slap_bass'")
    drum_groove: str = Field(description="Must be one of: 'none', 'hihat_only', 'standard_rock', 'four_on_the_floor', 'half_time', 'syncopated_funk', 'jazz_swing', 'bossa_nova_groove', 'lofi_hiphop', 'edm_four_on_the_floor', 'rnb_groove', 'cinematic_percussion'")
    choir_style: str = Field(default="pop_oohs_aahs", description="Must be one of: 'pop_oohs_aahs', 'classical_polyphony', 'romantic_homophony', 'modern_gospel'")
...
def generate_arrangement(prompt: str, style: str, key: str, bpm: int, num_sections: int = 1, lead_melody: list = None, is_simple_mode: bool = False) -> dict:
    ...
    system_instruction = (
        ...
        "Select the most appropriate `piano_rhythm`, `bass_rhythm`, `drum_groove`, and `choir_style` that perfectly matches the requested Style (e.g., use classical polyphony for Classical, modern gospel for R&B, romantic homophony for Cinematic). "
...
def generate_pattern_tracks(patterns: list, beats_per_measure: int, style: str, prompt: str, lead_melody: list = None) -> list:
    import time
    ...
    for measure_idx, pat in enumerate(patterns):
        ...
        # --- HELPER DYNAMIC RHYTHM ---
        def has_melody_note_at(beat, window=0.15):
            if not lead_melody: return False
            for n in lead_melody:
                if abs(n.get("startTime", 0) - beat) < window:
                    return True
            return False

        def is_melody_active(start_b, end_b):
            if not lead_melody: return False
            for n in lead_melody:
                nst = n.get("startTime", 0)
                nend = nst + n.get("duration", 0)
                if max(start_b, nst) < min(end_b, nend):
                    return True
            return False
        ...
        elif b_rhythm == 'bossa_nova_bass':
            fifth = chord_pitches[2] - 24 if chord_pitches[2] >= 60 else chord_pitches[2] - 12
            add_note(bass_track, bass_pitch, base_beat, 1.4, m_str)
            add_note(bass_track, fifth, base_beat + 1.5, 0.5, m_str)
            add_note(bass_track, fifth, base_beat + 2.0, 1.4, m_str)
            add_note(bass_track, bass_pitch, base_beat + 3.5, 0.5, m_str)
        elif b_rhythm == 'edm_offbeat_bass':
            for b in range(4):
                add_note(bass_track, bass_pitch, base_beat + b + 0.5, 0.4, m_str)
        elif b_rhythm == 'lofi_sub_bass':
            add_note(bass_track, bass_pitch - 12, base_beat, 2.5, m_str) # deep sub
            if not has_melody_note_at(base_beat + 3.0):
                add_note(bass_track, bass_pitch - 12, base_beat + 3.0, 0.8, m_str)
        elif b_rhythm == 'rnb_slap_bass':
            add_note(bass_track, bass_pitch, base_beat, 0.8, m_str)
            add_note(bass_track, bass_pitch + 12, base_beat + 0.75, 0.2, m_str) # octave pop
            add_note(bass_track, bass_pitch, base_beat + 1.5, 0.4, m_str)
            add_note(bass_track, chord_pitches[2] - 24, base_beat + 2.5, 0.8, m_str)
            add_note(bass_track, bass_pitch, base_beat + 3.5, 0.4, m_str)
        else: # syncopated_pop
            add_note(bass_track, bass_pitch, base_beat, 1.4, m_str)
            add_note(bass_track, bass_pitch, base_beat + 1.5, 0.9, m_str)
            # Dynamic bass interaction
            if not has_melody_note_at(base_beat + 3.0):
                add_note(bass_track, bass_pitch, base_beat + 2.5, 0.4, m_str)
                add_note(bass_track, chord_pitches[2] - 24, base_beat + 3.0, 0.9, m_str)
            else:
        ...
        if d_groove == 'none':
        ...
        elif d_groove == 'bossa_nova_groove':
            add_note(drums_track, 36, base_beat, 0.5, m_str)
            add_note(drums_track, 36, base_beat + 1.5, 0.5, m_str)
            add_note(drums_track, 36, base_beat + 2.0, 0.5, m_str)
            add_note(drums_track, 36, base_beat + 3.5, 0.5, m_str)
            for b in range(8):
                add_note(drums_track, 42, base_beat + (b * 0.5), 0.25, m_str)
            add_note(drums_track, 37, base_beat, 0.5, m_str)
            add_note(drums_track, 37, base_beat + 1.5, 0.5, m_str)
            add_note(drums_track, 37, base_beat + 2.5, 0.5, m_str)
        elif d_groove == 'lofi_hiphop':
            add_note(drums_track, 36, base_beat, 0.5, m_str, humanize=True)
            add_note(drums_track, 36, base_beat + 2.5, 0.5, m_str, humanize=True)
            add_note(drums_track, 38, base_beat + 1.05, 0.5, m_str, humanize=True) # late snare
            add_note(drums_track, 38, base_beat + 3.05, 0.5, m_str, humanize=True)
            for b in range(8):
                add_note(drums_track, 42, base_beat + (b * 0.5) + 0.03, 0.2, m_str, humanize=True) # swung hihat
        elif d_groove == 'edm_four_on_the_floor':
            for b in range(4):
                add_note(drums_track, 36, base_beat + b, 0.5, m_str) # Kick
                add_note(drums_track, 46, base_beat + b + 0.5, 0.25, m_str) # Open hi-hat
            add_note(drums_track, 39, base_beat + 1, 0.5, m_str) # Clap
            add_note(drums_track, 39, base_beat + 3, 0.5, m_str) # Clap
        elif d_groove == 'rnb_groove':
            add_note(drums_track, 36, base_beat, 0.5, m_str)
            add_note(drums_track, 36, base_beat + 1.75, 0.25, m_str)
            add_note(drums_track, 36, base_beat + 2.5, 0.5, m_str)
            add_note(drums_track, 38, base_beat + 1, 0.5, m_str)
            add_note(drums_track, 38, base_beat + 3, 0.5, m_str)
            for b in range(16):
                add_note(drums_track, 42, base_beat + (b * 0.25), 0.1, m_str)
        elif d_groove == 'cinematic_percussion':
            add_note(drums_track, 41, base_beat, 1.5, m_str) # Low Floor Tom
            if measure_idx % 2 == 1:
                add_note(drums_track, 41, base_beat + 2.0, 1.5, m_str)
            else:
                add_note(drums_track, 49, base_beat, 2.0, m_str) # Crash
        else: # standard_rock
            for b in range(4):
            ...
            for b in range(8):
                add_note(drums_track, 42, base_beat + (b * 0.5), 0.25, m_str)
            # Add crash on section start
            if measure_idx % 4 == 0:
                add_note(drums_track, 49, base_beat, 1.0, m_str) # Crash
            # Dynamic Drum Fill if melody is silent at end of 4-bar phrase
            if measure_idx % 4 == 3 and not is_melody_active(base_beat + 2.0, base_beat + 4.0):
                add_note(drums_track, 47, base_beat + 2.0, 0.25, m_str) # Tom
                add_note(drums_track, 47, base_beat + 2.5, 0.25, m_str)
                add_note(drums_track, 45, base_beat + 3.0, 0.25, m_str) # Low Tom
                add_note(drums_track, 43, base_beat + 3.5, 0.25, m_str) # Floor Tom
        ...
        if p_rhythm == 'block_whole':
            ...
            for b in range(8):
                # Accent the 8th notes where melody falls
                vol_mod = 0.9 if has_melody_note_at(base_beat + (b * 0.5)) else 0.6
                for p in vl_pitches:
                    add_note(piano_track, p, base_beat + (b * 0.5), 0.3, m_str)
        elif p_rhythm == 'bossa_nova_comping':
            offsets = [0.0, 1.5, 2.5] if measure_idx % 2 == 0 else [0.5, 2.0, 3.5]
            for offset in offsets:
                for p in vl_pitches:
                    add_note(piano_track, p, base_beat + offset, 0.4, m_str)
        elif p_rhythm == 'edm_synth_chords':
            for b in range(4):
                for p in vl_pitches:
                    add_note(piano_track, p, base_beat + b + 0.25, 0.25, m_str)
                    add_note(piano_track, p, base_beat + b + 0.5, 0.25, m_str)
                    add_note(piano_track, p, base_beat + b + 0.75, 0.25, m_str)
        elif p_rhythm == 'lofi_chords':
            for p in vl_pitches:
                add_note(piano_track, p, base_beat + 0.05, 1.5, m_str, humanize=True)
            if not has_melody_note_at(base_beat + 2.5):
                for p in vl_pitches:
                    add_note(piano_track, p, base_beat + 2.55, 1.0, m_str, humanize=True)
        elif p_rhythm == 'acoustic_strumming':
            for b in range(8):
                for i, p in enumerate(vl_pitches):
                    strum_delay = i * 0.03 if b % 2 == 0 else (len(vl_pitches) - 1 - i) * 0.03
                    add_note(piano_track, p, base_beat + (b * 0.5) + strum_delay, 0.4, m_str)
        elif p_rhythm == 'classical_arpeggio':
            alp_pitches = [vl_pitches[0], vl_pitches[-1], vl_pitches[1] if len(vl_pitches)>1 else vl_pitches[0], vl_pitches[-1]]
            for b in range(4):
                for i, p in enumerate(alp_pitches):
                    add_note(piano_track, p, base_beat + b + (i * 0.25), 0.25, m_str)
        elif p_rhythm == 'rnb_neo_soul_chords':
            for i, p in enumerate(vl_pitches):
                add_note(piano_track, p, base_beat + 0.1, 1.8, m_str)
                add_note(piano_track, p, base_beat + 2.6, 1.2, m_str)
            add_note(piano_track, vl_pitches[-1] + 2, base_beat + 1.8, 0.2, m_str)
        else: # block_syncopated
            offsets = [0.0, 1.5, 2.5]
            # Dynamic syncopation based on melody
            if has_melody_note_at(base_beat + 1.0) and not has_melody_note_at(base_beat + 1.5):
                offsets = [0.0, 1.0, 2.5]
            ...
            # Fill space if melody is silent
            if not is_melody_active(base_beat + 2.5, base_beat + 4.0):
                arp_p = vl_pitches[2] if len(vl_pitches)>2 else vl_pitches[0]+7
                add_note(piano_track, arp_p, base_beat + 3.0, 0.5, m_str)
                add_note(piano_track, vl_pitches[1] if len(vl_pitches)>1 else vl_pitches[0]+4, base_beat + 3.5, 0.5, m_str)
                    
        # --- SATB CHOIR GENERATION (Algorithmic Voice Led) ---
        if needs_satb:
            c_style = pat.get('choir_style', 'pop_oohs_aahs') if isinstance(pat, dict) else 'pop_oohs_aahs'
            ...
            if c_style == 'classical_polyphony':
                # Staggered entries and passing notes
                # Soprano/Bass enter on 1, Alto/Tenor enter on 2
                add_note(satb_tracks["Soprano"], soprano_p, base_beat, 3.8, m_str, "Ah", humanize=True)
                add_note(satb_tracks["Bass"], bass_vocal_p, base_beat, 3.8, m_str, "Ah", humanize=True)
                
                # Alto passing note on beat 3.5
                add_note(satb_tracks["Alto"], alto_p, base_beat + 1.0, 2.5, m_str, "Ah", humanize=True)
                add_note(satb_tracks["Alto"], alto_p - 1, base_beat + 3.5, 0.5, m_str, "Ah", humanize=True)
                
                add_note(satb_tracks["Tenor"], tenor_p, base_beat + 1.0, 2.8, m_str, "Ah", humanize=True)
                
            elif c_style == 'romantic_homophony':
                # Sweeping lush chords slightly overlapping (legato)
                dur = 4.2 # slight overlap into next measure
                add_note(satb_tracks["Soprano"], soprano_p, base_beat, dur, m_str, "Ooh", humanize=True)
                add_note(satb_tracks["Alto"], alto_p, base_beat, dur, m_str, "Ooh", humanize=True)
                add_note(satb_tracks["Tenor"], tenor_p, base_beat, dur, m_str, "Ooh", humanize=True)
                add_note(satb_tracks["Bass"], bass_vocal_p, base_beat, dur, m_str, "Ooh", humanize=True)
                
            elif c_style == 'modern_gospel':
                # Tight, rhythmic stabs on offbeats
                rhythm = [0.0, 1.5, 2.5]
                for offset in rhythm:
                    dur = 0.5
                    add_note(satb_tracks["Soprano"], soprano_p, base_beat + offset, dur, m_str, "Yeah", humanize=True)
                    add_note(satb_tracks["Alto"], alto_p, base_beat + offset, dur, m_str, "Yeah", humanize=True)
                    add_note(satb_tracks["Tenor"], tenor_p, base_beat + offset, dur, m_str, "Yeah", humanize=True)
                    add_note(satb_tracks["Bass"], bass_vocal_p, base_beat + offset, dur, m_str, "Yeah", humanize=True)
...
def generate_arrangement_with_midi(payload: dict) -> dict:
    ...
    final_tracks = generate_pattern_tracks(patterns, beats_per_measure, style, prompt, lead_melody)
