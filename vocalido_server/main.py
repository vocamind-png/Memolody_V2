...
# ── Render File Management Config ─────────────────────────────────────────────
RENDER_DIR = "renders"
RENDER_TTL_HOURS = 24          # Auto-delete renders older than this
RENDER_MAX_DISK_MB = 2000      # Max total render disk usage (2 GB)
...
def _studio_preview_impl(req: StudioPreviewReq):
    ...
    if req.owner_id:
        _enforce_user_render_quota(req.owner_id)
    if req.song_id:
...
@app.get("/studio/renders-stats")
def renders_stats():
    """Return render disk usage statistics for monitoring."""
    os.makedirs(RENDER_DIR, exist_ok=True)
    total_size = 0
    file_count = 0
    oldest_age = 0
    owner_counts: dict = {}  # owner_id → count
    now = time.time()

    for f in os.listdir(RENDER_DIR):
        fpath = os.path.join(RENDER_DIR, f)
        if not os.path.isfile(fpath) or not f.endswith((".mp3", ".wav")):
            continue
        stat = os.stat(fpath)
        ...
        # Count per-user renders (exclude stems)
        if f.startswith("song_") and "_stem_" not in f:
            # Try to extract owner from private format: song_{owner}_{id}_...
            parts = f.split("_")
            if len(parts) >= 3:
                owner_counts[parts[1]] = owner_counts.get(parts[1], 0) + 1

    return {
        "ok": True,
        "total_files": file_count,
        "total_size_mb": round(total_size / (1024 * 1024), 1),
        "max_disk_mb": RENDER_MAX_DISK_MB,
        "disk_usage_pct": round((total_size / (RENDER_MAX_DISK_MB * 1024 * 1024)) * 100, 1) if RENDER_MAX_DISK_MB else 0,
        "oldest_file_hours": round(oldest_age / 3600, 1),
        "ttl_hours": RENDER_TTL_HOURS,
        "max_per_user": RENDER_MAX_PER_USER,
        "top_users": dict(sorted(owner_counts.items(), key=lambda x: -x[1])[:10]),
    }
...
async def lyria_render(request: Request):
    ...
    try:
        ...
        gen_mode = data.get("mode", "full")
        notes = data.get("notes", [])
        prompt = data.get("prompt", "Pop Music")
        bpm = data.get("bpm", 120)
        lyrics = data.get("lyrics", "").strip()
        key = data.get("key", "C")
        
        if gen_mode == "backing" and not notes:
...
async def extract_chords(request: Request):
    try:
        ...
        if not audio_b64 or not lyrics:
            return JSONResponse({"success": False, "message": "Missing audio_base64 or lyrics"}, status_code=400)
            
        print("[API] Running chord extraction pipeline...")
        annotated_lyrics = extract_chords_from_audio(audio_b64, lyrics)
        
        return JSONResponse({
            "success": True,
            "message": "Chords extracted successfully.",
            "data": {"lyrics": annotated_lyrics}
        })
    except Exception as e:
        print(f"[Extract Chords Error] {e}")
        return JSONResponse({"success": False, "message": str(e)}, status_code=500)


@app.post("/vocalido/api/ai/transcribe-audio")
async def transcribe_audio(request: Request):
    try:
        from lyrics_transcriber import transcribe_audio_to_lyrics
        data = await request.json()
        audio_b64 = data.get("audio_base64")
        youtube_url = data.get("youtube_url")
        
        if not audio_b64 and not youtube_url:
            return JSONResponse({"success": False, "message": "Missing audio_base64 or youtube_url"}, status_code=400)
            
        print(f"[API] Running audio transcription pipeline (YouTube: {bool(youtube_url)})...")
        transcribed_lyrics = transcribe_audio_to_lyrics(audio_b64, youtube_url)
        
        return JSONResponse({
            "success": True,
            "message": "Audio transcribed successfully.",
            "data": {"lyrics": transcribed_lyrics}
        })
    except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════════════════
# RENDER FILE MANAGEMENT — Auto-cleanup & User Quota
# ═══════════════════════════════════════════════════════════════════════════════
import asyncio as _asyncio

def _enforce_user_render_quota(owner_id: str):
    """Delete oldest renders for a user if they exceed RENDER_MAX_PER_USER."""
    import re as _re_q
    safe_owner = _re_q.sub(r'[^a-zA-Z0-9_-]', '_', owner_id)[:40]
    prefix = f"song_{safe_owner}_"

    user_files = []
    try:
        for f in os.listdir(RENDER_DIR):
            if f.startswith(prefix) and "_stem_" not in f and f.endswith((".mp3", ".wav")):
                fpath = os.path.join(RENDER_DIR, f)
                user_files.append((fpath, os.path.getmtime(fpath)))
    except FileNotFoundError:
        return

    if len(user_files) < RENDER_MAX_PER_USER:
        return

    # Sort oldest first
    user_files.sort(key=lambda x: x[1])
    to_delete = user_files[:len(user_files) - RENDER_MAX_PER_USER + 1]

    for old_path, _ in to_delete:
        try:
            base = os.path.splitext(os.path.basename(old_path))[0]
            os.remove(old_path)
            # Also remove associated stems
            for sf_name in os.listdir(RENDER_DIR):
                if sf_name.startswith(f"{base}_stem_"):
                    os.remove(os.path.join(RENDER_DIR, sf_name))
            print(f"[Quota] 🗑️ Evicted old render: {os.path.basename(old_path)} "
                  f"(user {safe_owner} exceeded {RENDER_MAX_PER_USER} limit)")
        except Exception as e:
            print(f"[Quota] ⚠️ Failed to evict: {e}")


async def _cleanup_old_renders():
    """Background task: delete renders older than TTL and enforce disk quota."""
    await _asyncio.sleep(10)  # Wait for server to fully start
    while True:
        try:
            now = time.time()
            ttl_seconds = RENDER_TTL_HOURS * 3600
            total_size = 0
            files_with_age = []
            ...
            for f in os.listdir(RENDER_DIR):
                ...
                age = now - stat.st_mtime
                total_size += stat.st_size
                files_with_age.append((fpath, age, stat.st_size, f))

            deleted_count = 0
            freed_bytes = 0

            # Pass 1: Delete files older than TTL
            still_alive = []
            for fpath, age, size, fname in files_with_age:
                if age > ttl_seconds:
                    try:
                        os.remove(fpath)
                        total_size -= size
                        ...
                        still_alive.append((fpath, age, size, fname))
                else:
                    still_alive.append((fpath, age, size, fname))

            # Pass 2: If disk still exceeds quota, evict oldest first (LRU)
            max_bytes = RENDER_MAX_DISK_MB * 1024 * 1024
            if total_size > max_bytes:
                still_alive.sort(key=lambda x: -x[1])  # oldest first
                for fpath, age, size, fname in still_alive:
                    if total_size <= max_bytes:
                        break
                    try:
                        os.remove(fpath)
            ...
            if deleted_count:
                print(f"[Cleanup] 🗑️ Deleted {deleted_count} old renders, "
                      f"freed {freed_bytes / (1024*1024):.1f} MB. "
                      f"Remaining: {total_size / (1024*1024):.0f} MB")

        except Exception as e:
            print(f"[Cleanup] ⚠️ Error during render cleanup: {e}")
...
@app.post("/api/ai/tts")
async def text_to_speech_cloud(request: Request):
    try:
        data = await request.json()
        text = data.get("text", "")
        voice_gender = data.get("gender", "female") # "female" or "male"
        lang = data.get("lang", "th") # "th", "en", "ja", "zh", "ko", etc.
        ...
        # Select best Microsoft Edge Neural voice
        voice_map = {
            "th": {
        ...
        lang_group = voice_map.get(lang, voice_map["en"])
        selected_voice = lang_group.get(voice_gender, lang_group["female"])
        
        # Output file path in renders directory
        ...
        # Return URL to the mp3 file
        return {"url": f"/vocalido/audio/{filename}"}
        
    except ImportError:
        # Fallback if edge-tts not installed
        print("[TTS] edge-tts package not found, please run: pip install edge-tts")
        raise HTTPException(status_code=501, detail="edge-tts not installed on server")
    except Exception as e:
        print(f"[TTS Error] {e}")
...
@app.on_event("startup")
async def _start_render_cleanup():
    """Start the background render cleanup task on server boot."""
    _asyncio.create_task(_cleanup_old_renders())
    print(f"[Cleanup] ✅ Render auto-cleanup started "
          f"(TTL={RENDER_TTL_HOURS}h, max={RENDER_MAX_DISK_MB}MB, "
          f"quota={RENDER_MAX_PER_USER}/user, interval={RENDER_CLEANUP_INTERVAL}s)")
